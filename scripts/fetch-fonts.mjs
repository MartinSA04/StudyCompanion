 ...
 * local("Georgia") sized and shaped to the real web font so the fallback-swap
 * causes minimal reflow (the token-stack entries that reference these names
 * are wired in tokens.css by the chrome package).
 *
 * size-adjust = webFontAvgAdvance / georgiaAvgAdvance, where each AvgAdvance is
 ...
 *
 * Source metrics (all from @capsizecss/metrics@4.1.0, cross-checked against
 * the shipped woff2 via `fonttools` — unitsPerEm/ascent/descent/lineGap match
 * exactly, confirming the same font version):
...
const out = [
  ...
  ' * the upright weights. "Spectral Fallback" / "Fraunces Fallback" below are',
  ' * metrics-matched local("Georgia") faces, not part of the Google fetch.',
  " */",
  "",
];
let kept = 0;
