...
- On every release: bump `package.json` version, then update the framework pin to
  the newest tag in `course-template/` AND every consuming course repo (optics at
  `~/School/TFY4195/companion`, algdat at `~/School/TDT4120/companion`). Keep all
  course pins in lockstep with the latest tag.
